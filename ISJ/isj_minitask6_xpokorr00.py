...
# Define function add_question with 3 parameters - index of the question that should be added, 
# list of the pairs (question, list of possible answers), and the current quiz. 
...
#  a new quiz with just the selected question is returned - the default of the third parameter
#  is thus an empty quiz.

def add_question(index: int, qpool: list, quiz: list = None) -> list:
    if index < 0 or index >= len(qpool):
        print("Invalid index!")
        return quiz if quiz is not None else []
